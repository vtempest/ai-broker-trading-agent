...
export async function inviteMemberToTeam(teamId: string, email: string) {
  try {
    ...
    if (
      team.upgradeMembers &&
      team.members &&
      team.members.length >= (team.maxMembers || 8)
    ) {
      return {
        success: false,
        error: `Team is full. Maximum ${
          team.maxMembers || 8
        } members allowed for Pro teams.`,
      };
    }
    ...
    if (upgradeMembers === true) {
      ...
      if (team && team.members && team.members.length > 8) {
        return {
          success: false,
          error:
            "Team has more than 8 members. Remove members before enabling Pro upgrades.",
        };
      }
    }
