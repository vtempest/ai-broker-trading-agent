...
import { fetchMarketSummary, fetchMarketsDashboard } from './polymarket'
 ...
 */
export async function getMarketSummary(eventId: string): Promise<MarketSummary> {
  try {
    const data = await fetchMarketSummary(eventId)
    return data as MarketSummary
  } catch (error) {
    console.error(`Failed to fetch market summary for event ${eventId}:`, error)
    throw error
  }
}
 ...
 */
export async function getMarketsDashboard(eventId: string): Promise<DashboardData> {
  try {
    const data = await fetchMarketsDashboard(eventId)
    return data as DashboardData
  } catch (error) {
    console.error(`Failed to fetch dashboard data for event ${eventId}:`, error)
    throw error
  }
}
...
export async function exampleDashboard() {
  try {
    ...
    console.log("Dashboard Highlights:", {
      ...
      recentPrice: dashboard.charts?.[dashboard.charts.length - 1]?.price || 'N/A',
    })
